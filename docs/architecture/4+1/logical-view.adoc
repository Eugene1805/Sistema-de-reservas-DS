...
=== Paquete: Business Services (Capa de Negocio)

*Responsabilidad*::
*Corazón del sistema* - contiene lógica de dominio y reglas de negocio

*Servicios*::
* `ReservationService`:: Orquesta proceso de reserva, políticas de precios, cancelaciones
* `InventoryService`:: Gestiona disponibilidad de habitaciones, hoteles, lógica de overbooking
* `PaymentService`:: Integración con pasarelas de pago externas
* `HotelManagementService`:: Funcionalidad de backoffice para administración

=== Paquete: Infrastructure (Capa de Infraestructura/Datos)

*Responsabilidad*::
Capacidades técnicas transversales

*Componentes*::
* `DataAccess`:: Implementa patrón Repositorio (ej. `IReservationRepository`, `SqlReservationRepository`)
* `Messaging`:: Abstracción sobre Broker de mensajería para eventos (ej. `ReservationCreatedEvent`)
* `SharedKernel`:: Elementos de dominio compartidos (ej. `Money`, `DateRange`, entidades transversales)
// Add diagrams here
