...
| xref:process-view[Vista de Procesos], Paquete *Mensajería*, Pod Workers, ServicioDeReservas (flujo asíncrono)
| Cumple

| QAS-02 (Desempeño) – Consultas P95 ≤ 500 ms a 300 QPS
| xref:dec-17[Flujos sin bloqueo] +
  xref:dec-2[Arquitectura en capas]
| ServicioDeInventario, xref:logical-view[Vista Lógica], xref:process-view[Vista de Procesos]
| Cumple
...
| xref:physical-view[Vista Física] (Cluster SGH), Nodos Front-End, Pods distribuidos
| Cumple
...
| xref:physical-view[Vista Física] (API Gateway en DMZ), Certificados SSL/TLS, Política de acceso
| Cumple
...
| xref:dec-11[Redundancia por clustering] +
  xref:dec-16[Servicios stateless]
| xref:physical-view[Vista Física] (Pods replicados, cluster activo-activo)
| Cumple

// ---------------- QAS-07 / CRN-02 ----------------
| QAS-07 (Disponibilidad) & CRN-02 (Evitar pagos dobles)
| xref:dec-7[Idempotencia en pagos]
| ServicioDePago, Registro de transacciones, xref:logical-view[Vista Lógica]
| Cumple
...
| xref:logical-view[Vista Lógica] (Presentación/Servicios/Datos)
| Cumple
...
| ServicioDeReservas (Orquestador Saga), ServicioDeInventario (bloqueos), ServicioDePagos; xref:process-view[Vista de Procesos]
| Cumple

// ---------------- CON-04 ----------------
| CON-04 – Uso obligatorio de RDBMS
| xref:dec-1[MySQL (RDBMS)] +
  xref:dec-14[Patrón Repositorio]
| Componente Acceso a Datos, ServicioDeReservas, ServicioDeInventario, ServicioDePagos; xref:physical-view[Vista Física] (MySQL Master)
| Cumple
...
| xref:dec-10[Exclusión mutua en inventario] +
  xref:dec-6[Saga]
| ServicioDeInventario (locks), ServicioDeReservas (coordinación), xref:process-view[Vista de Procesos]
| Cumple
...
| xref:dec-17[Optimización de consultas] +
  xref:dec-2[Arquitectura en capas]
| xref:functional-view[Vista Funcional], xref:logical-view[Vista Lógica], ServicioDeInventario
| Cumple

// ---------------- Overbooking ----------------
| Política de Overbooking del 10%
| xref:dec-10[Control de concurrencia en inventario]
| ServicioDeInventario (regla de negocio), xref:logical-view[Vista Lógica]
| Cumple

// ---------------- Redis (rendimiento) ----------------
| QAS-02 (Desempeño) – Aceleración de lectura de disponibilidad
| xref:dec-17[Optimización de consultas] +
  Uso de Redis como caché (patrón Cache Aside)
| ServicioDeInventario, xref:physical-view[Vista Física] (Redis Cache), xref:logical-view[Vista Lógica]
| Cumple

// ---------------- Auditoría ----------------
| CRN-01 (Privacidad) + Necesidad de trazabilidad operativa
| xref:dec-15[Observabilidad: logs, métricas, trazas]
| Paquete Auditoría, Logs distribuidos, xref:process-view[Vista de Procesos]
| Cumple

// ---------------- Workers ----------------
| QAS-02 (Desempeño) – Desacoplar procesamiento lento
| xref:dec-9[Workers asíncronos] +
  xref:dec-8[Message Broker]
| Pod Workers, Broker, xref:process-view[Vista de Procesos]
| Cumple
